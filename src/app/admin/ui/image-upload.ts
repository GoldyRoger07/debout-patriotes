 ...
 * déjà présente dans la médiathèque, et expose `{ url, fileId }` comme valeur.
 *
...
@Component({
  ...
  template: `
    <div class="flex flex-col gap-3">
      ...
      >
        @if (value(); as image) {
          <img [src]="image.url | ik: 'w-800'" alt="Aperçu" class="h-full w-full object-cover" />
        } @else {
          <div
...
export class ImageUpload implements ControlValueAccessor, OnDestroy {
  ...
  /** Classe de proportion de la zone d'aperçu. */
  readonly aspect = input('aspect-video');
  ...
  protected async pickFromGallery(): Promise<void> {
    ...
    if (image) {
      ...
      this.update({ url: image.url, fileId: image.fileId });
    }
  }
